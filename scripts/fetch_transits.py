...
def jd_ut(dt): return swe.julday(dt.year, dt.month, dt.day,
...
def swiss_lonlat(eid, jd):
    try:
        ...
        if body is None: body = int(eid)
        xx, _ = swe.calc_ut(jd, body)
        return float(xx[0]), float(xx[1]), "swiss"
    except: return None, None, None

def load_fallback():
    try: return {row["date"]: row for row in json.load(open(FALLBACK_PATH))["data"]}
    except: return {}
...
def normalize_obj(obj):
    return {
        ...
        "ecl_lon_deg": obj.get("ecl_lon_deg", None),
...
def main():
    ...
    jd = jd_ut(now); date_key = now.strftime("%Y-%m-%d")
    fallback = load_fallback()

    feed = {"generated_at_utc": now.isoformat()+"Z", "observer":"geocentric Earth",
            "source":"JPL Horizons + Swiss + fallback JSON", "objects":[]}
    ...
    for p in cfg["planets"]:
        ...
        feed["objects"].append(normalize_obj({"id":eid,"targetname":label,
            "ecl_lon_deg":lon,"ecl_lat_deg":lat,"source":src}))

    for name,num in ASTEROIDS_TNOS.items():
        lon, lat, src = swiss_lonlat(str(num), jd)
        if lon is not None:
            feed["objects"].append(normalize_obj({"id":str(num),"targetname":name,
                "ecl_lon_deg":lon,"ecl_lat_deg":lat,"source":src}))
        elif date_key in fallback and name in fallback[date_key]:
            feed["objects"].append(normalize_obj({"id":name,"targetname":name,
                "ecl_lon_deg":fallback[date_key][name],"ecl_lat_deg":0.0,
                "source":"fallback-json"}))
        else:
            feed["objects"].append(normalize_obj({"id":name,"targetname":name,"source":"missing"}))

    for star in cfg["fixed_stars"]:
        feed["objects"].append(normalize_obj({"id":star["id"],"targetname":star["label"],
            "ecl_lon_deg":star["ra_deg"],"ecl_lat_deg":star["dec_deg"],"source":"fixed"}))
